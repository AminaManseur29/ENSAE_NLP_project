...
## 🚀 Features
This project offers a pipeline for text classification, from feature extraction to model training, evaluation, and deployment.

### 🔎 Feature Extraction Methods
- BERT Embeddings (bert-base-cased)
    → Generates deep semantic vector representations of texts using the BERT model.

- RoBERTa Embeddings (roberta-base)
    → Uses the RoBERTa transformer model to produce rich contextual embeddings.

- SentenceBERT Embeddings (all-MiniLM-L6-v2)
    → Computes efficient sentence-level embeddings optimized for semantic similarity tasks.

- Handcrafted Features
    → Manually designed features based on:
...
- Bag of Words (BoW)
    → Classic representation based on token counts, capturing the presence of specific words.
...
- Saving Trained Models and Scalers:
    - Each trained model and associated feature scaler is saved separately for reproducibility and easy deployment.
